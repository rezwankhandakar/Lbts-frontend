/**
 * The seam between this app and a physical scanner.
 *
 ...
 *
 * Everything above this file is written against `ScannerAgent`, so no feature
 * learns what is on the other end. Replacing WIA with a vendor SDK, or with a
 * different transport entirely, is a new implementation of this interface and
 * no change to a single component.
 *
 * It lived in `features/gate-pass/lib/` until the Vendor module needed to scan
 * a compliance document, and moved out rather than being copied — the rule
 * CLAUDE.md sets for a helper a second feature wants. The pairing in
 * localStorage is per workstation, not per feature, so two features scanning
